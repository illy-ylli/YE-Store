...
// merri produktet Home & Kitchen (category_id = 2)
$stmt = $conn->prepare("SELECT * FROM products WHERE category_id = 2 ORDER BY created_at DESC");
$stmt->execute();
$homeKitchenProducts = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Produkte Shtëpiake & Kuzhinë - Y/E Store</title>
...
<nav class="navbar">
    ...
    <main>
        <section class="products-section">
            <h2>Shtëpiake & Kuzhinë</h2>
            <div class="products-count">
                <?= count($homeKitchenProducts) ?> product(s) found
            </div>
            <div class="product-row">
                <?php if (count($homeKitchenProducts) > 0): ?>
                    <?php foreach ($homeKitchenProducts as $product): ?>
                ...
                <?php else: ?>
                    <div class="product-card placeholder">
                        <div class="product-image">
                            <img src="images/nothinghere.png" alt="No Home & Kitchen Products">
                        </div>
                        <div class="product-footer">
                            <h3>Nuk ka produkte për Shtëpi & Kuzhinë në dispozicion</h3>
